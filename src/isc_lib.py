...
from pysyncon import Dataprep, Synth, PenalizedSynth, AugSynth, RobustSynth
import warnings
import tqdm 
...
def sc(data_object, 
    ...
    if reduction:
        ...
        try:
            ...
            return None
    melted_df = data.reset_index().melt(id_vars=['var', 'year'], var_name='pid')
    pivoted_df = melted_df.pivot(index=['year', 'pid'], columns='var', values='value').reset_index()
    pivoted_df =pivoted_df.sort_values(by=['pid', 'year'])
    pivoted_df = pivoted_df.reset_index(drop=True) 
    pivoted_df.columns.name = ''
    controls = pivoted_df.pid[pivoted_df.pid!=treated_unit].unique().tolist()
    covariates = pivoted_df.columns.to_list()
    covariates.remove('year')
    covariates.remove('pid')       
    try:
        dataprep = Dataprep(
            foo=pivoted_df,
    ...
    try:
        if penalized:
            ...
            synth.fit(dataprep=dataprep, lambda_=lambda_, sv_count=3)
        else:
            synth = Synth()
            synth.fit(dataprep)
...
def isc(data_objects: list, 
    ...
    placebo_avs = []
    with Pool() as p:
        out = p.starmap(sc, tqdm.tqdm([(data, penalized, reduction, k_n, lambda_, placebo, seed) for data in data_objects]))
